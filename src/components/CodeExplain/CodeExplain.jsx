    ...
    return (
        <div>
            ...
            <div className='flex flex-col mb-14'>
                ...
                After constructing the job object, it logs the newly created job for debugging purposes. Then, it sends a POST request to a specified endpoint (https://job-wander-server.vercel.app/job) with the new job data in JSON format. This request includes headers specifying the content type as JSON. Upon receiving a response from the server, it parses the response JSON and logs it. If the response indicates that the job was successfully added (e.g., contains an insertedId property), it displays a success message using the Swal (SweetAlert) library.</p>
            </div>
        </div>
    );
};
